...
/* ════════════════════════════════════════════════════════════════════════════
   طابور — مركز المهام (v19)
   • كتالوج المهام اليومية قابل للتعديل والإضافة والحذف من مدير النظام
   • المهام المستعجلة موزَّعة حسب التسلسل الوظيفي
   • الفروع مربوطة بموقع جغرافي إلزامي يغذّي جداول الدوام والحضور
   ════════════════════════════════════════════════════════════════════════════ */
const tbEl = me.createElement,
  TB_ROLES = ["ops_manager", "area_manager", "branch_manager", "cashier"],
  TB_LEVEL = {
    ops_manager: 0,
    area_manager: 1,
    branch_manager: 2,
  ...
  TB_PERIODS = {
    anytime: { t: "طوال اليوم", i: "🕘" },
    morning: { t: "الافتتاح", i: "🌅" },
    evening: { t: "المساء", i: "🌆" },
    closing: { t: "الإغلاق", i: "🌙" }
  },
  TB_PRIO = { عالي: "#ef4444", متوسط: "#f59e0b", منخفض: "#80AFB2" },
  TB_STAT = { جديد: "#ef4444", "قيد التنفيذ": "#f59e0b", مكتمل: "#22c55e" },
  TB_DEF_RADIUS = 150,
  tbEnc = W => encodeURIComponent(W == null ? "" : String(W)),
  tbHeaders = W => ({
    "content-type": "application/json",
    "x-tabor-role": (W && W.role) || "",
    "x-tabor-region": tbEnc(W && W.region),
    "x-tabor-branch": tbEnc(W && W.branch),
    "x-tabor-user": tbEnc(W && W.name)
  }),
  /** نداء API موحّد يحمل هوية المستخدم (المنصب/المنطقة/الفرع) */
  tbApi = async (W, Q, p, H) => {
    ...
    if (!T.ok) throw new Error(L.error || "تعذّر تنفيذ الطلب");
    return L
  },
  /** التسلسل الوظيفي: لا يُسند أحد مهمة إلا لمنصب أدنى منه */
  tbCanAssign = (W, Q) => TB_LEVEL[W] != null && TB_LEVEL[Q] != null && TB_LEVEL[Q] > TB_LEVEL[W],
  tbBelowRoles = W => TB_ROLES.filter(Q => tbCanAssign(W, Q)),
  tbNum = W => {
    if (W === "" || W == null) return null;
    const Q = Number(W);
    return Number.isFinite(Q) ? Q : null
  },
  /** توحيد صف المهمة القادم من الخادم أو من الافتراضيات المدمجة */
  tbNormTask = (W, Q) => ({
    dbId: W.dbId != null ? W.dbId : W.id != null && typeof W.id == "number" ? W.id : null,
    id: typeof W.id == "number" ? `t${W.id}` : W.id,
    role: W.role || Q || "",
    title: W.title || "",
    icon: W.icon || "📌",
    category: W.category || "عام",
    period: TB_PERIODS[W.period] ? W.period : "anytime",
    notes: W.notes || "",
    sortOrder: W.sortOrder != null ? W.sortOrder : W.sort_order != null ? W.sort_order : 0,
    active: W.active !== !1
  }),
  /** المهام الافتراضية المدمجة — تُستخدم عند تعذّر الاتصال بالخادم */
  tbFallbackTasks = () => Object.entries(hn).flatMap(([W, Q]) => Q.map((p, H) => tbNormTask({
    ...p,
    role: W,
    sortOrder: H
  }, W))),
  /** ترتيب المهام: الفترة ثم الترتيب اليدوي */
  tbSortTasks = W => [...W].sort((Q, p) => Q.sortOrder - p.sortOrder || Q.title.localeCompare(p.title, "ar")),
  /** تجميع المهام في مجموعات حسب التصنيف */
  tbGroupTasks = W => {
    const Q = [];
    return tbSortTasks(W).forEach(p => {
      const H = Q.find(T => T[0] === p.category);
      H ? H[1].push(p) : Q.push([p.category, [p]])
    }), Q
  },
  /** موقع الفرع: من بيانات الفرع أولاً ثم من الإحداثيات المدمجة */
  tbGeoOf = (W, Q) => {
    if (!Q) return null;
    for (const p in W) {
      const H = ((W[p] || {}).branches || []).find(T => T.id === Q);
      if (H && tbNum(H.lat) != null && tbNum(H.lng) != null) return {
        lat: tbNum(H.lat),
        lng: tbNum(H.lng),
        radius: tbNum(H.radius) || TB_DEF_RADIUS
      }
    }
    return af[Q] || null
  },
  /** الفروع التي لم يُحدَّد موقعها بعد — لا يمكن ربطها بجداول الدوام */
  tbBranchesNoGeo = W => Object.entries(W).flatMap(([Q, p]) => (p.branches || []).filter(H => tbNum(H.lat) == null || tbNum(H.lng) == null).map(H => ({
    ...H,
    region: Q,
    regionName: p.name
  }))),
  /** دمج فروع الخادم (مع إحداثياتها) داخل شجرة المناطق */
  tbMergeBranches = (W, Q) => {
    const p = {
      ...W
    };
    return Q.forEach(H => {
      const T = H.region;
      if (!p[T]) return;
      const L = (p[T].branches || []).slice(),
        ie = L.findIndex($ => $.id === H.branchId);
      const z = {
        id: H.branchId,
        name: H.name,
        supervisor: H.supervisor || (ie >= 0 ? L[ie].supervisor : ""),
        lat: H.lat,
        lng: H.lng,
        radius: H.radius,
        dbId: H.id
  ...
  tbDayName = ["أحد", "إثن", "ثلا", "أرب", "خمي", "جمع", "سبت"],
  /** آخر n يوم بصيغة ISO */
  tbLastDays = W => {
    const Q = [];
    for (let p = W - 1; p >= 0; p--) {
      const H = new Date;
      H.setDate(H.getDate() - p), Q.push(H.toISOString().split("T")[0])
    }
    return Q
  };

/** بطاقة الترويسة — تصميم مسطّح بهوية طابور مع شبكة خفيفة */
function TbHero(W) {
  const Q = 2 * Math.PI * 34;
  return tbEl("div", {
    style: {
      background: v.t,
  ...
  }, W.subtitle), W.note ? tbEl("p", {
    style: {
      fontSize: 11,
      opacity: .72,
      margin: "6px 0 0"
    }
  }, W.note) : null), tbEl("svg", {
    width: 82,
    height: 82,
    viewBox: "0 0 80 80"
  }, tbEl("circle", {
    ...
    strokeWidth: 7,
    strokeLinecap: "round",
    strokeDasharray: `${Math.max(0,Math.min(100,W.pct))/100*Q} ${Q}`,
...
function TbChips(W) {
  ...
  }, W.items.map(Q => tbEl("button", {
    key: Q.k,
    onClick: () => W.onPick(Q.k),
    style: {
      padding: "5px 12px",
      borderRadius: 20,
      border: `1px solid ${W.value===Q.k?v.t:"#e8eded"}`,
      background: W.value === Q.k ? v.t : "#fff",
      color: W.value === Q.k ? "#fff" : "#777",
      fontSize: 11.5,
      fontWeight: 600,
      cursor: "pointer"
    }
...
/** بطاقة مهمة مستعجلة */
function TbUrgentCard(W) {
  const Q = W.task,
    p = TB_PRIO[Q.priority] || v.t,
    H = TB_STAT[Q.status] || v.t,
    T = Q.status !== "مكتمل" && Q.dueDate && Q.dueDate < U();
  return tbEl("div", {
    style: {
      background: "#fff",
      borderRadius: 10,
  ...
  }, `⇩ ${qn[Q.targetRole]||Q.targetRole}`), T ? tbEl("span", {
    style: {
      ...a.bdg,
      background: "#ef444415",
      color: "#ef4444"
  ...
  }, Q.details) : null, tbEl("p", {
    style: {
      fontSize: 10,
      color: "#aaa",
      margin: 0
    }
  }, `👤 ${Q.createdByName||qn[Q.createdByRole]||"—"}`, W.scopeText ? ` • ${W.scopeText}` : "", Q.dueDate ? ` • 📅 ${Q.dueDate}` : ""), tbEl("div", {
    style: {
      display: "flex",
      gap: 5,
      marginTop: 8,
  ...
  }, Q.status !== "قيد التنفيذ" && Q.status !== "مكتمل" && (Q.assignedToMe || Q.canManage) ? tbEl("button", {
    disabled: W.busy,
    onClick: () => W.onStatus(Q, "قيد التنفيذ"),
    style: {
      ...a.stBtn,
      color: "#f59e0b"
    }
  }, "⏳ بدء التنفيذ") : null, Q.status !== "مكتمل" && (Q.assignedToMe || Q.canManage) ? tbEl("button", {
    disabled: W.busy,
    onClick: () => W.onStatus(Q, "مكتمل"),
    style: {
      ...a.stBtn,
      color: "#22c55e"
    }
  }, "✓ إنجاز") : null, Q.status === "مكتمل" && Q.canManage ? tbEl("button", {
    disabled: W.busy,
    onClick: () => W.onStatus(Q, "قيد التنفيذ"),
    style: {
      ...a.stBtn,
  ...
  }, "↩ إعادة فتح") : null, Q.canManage && W.onEdit ? tbEl("button", {
    onClick: () => W.onEdit(Q),
    style: {
      ...a.stBtn,
      color: v.t
    }
  }, "✎ تعديل") : null, Q.canManage && W.onDelete ? tbEl("button", {
    onClick: () => W.onDelete(Q),
    style: {
      ...a.stBtn,
      color: "#ef4444"
    }
...
/** نموذج مهمة يومية (إضافة/تعديل) — مدير النظام */
function TbTaskForm(W) {
  const Q = W.initial || {},
    [p, H] = me.useState({
      title: Q.title || "",
      icon: Q.icon || "📌",
      role: Q.role || W.defaultRole || "branch_manager",
      category: Q.category || "عام",
      period: Q.period || "anytime",
      notes: Q.notes || "",
      sortOrder: Q.sortOrder != null ? Q.sortOrder : W.nextOrder || 0,
      active: Q.active !== !1
    }),
    T = (ie, $) => H(z => ({
      ...z,
    ...
    L = !p.title.trim();
  return tbEl(me.Fragment, null, tbEl("div", {
    style: a.fg
  }, tbEl("label", {
    style: a.fl
  }, "عنوان المهمة *"), tbEl("input", {
    value: p.title,
    onChange: ie => T("title", ie.target.value),
    style: a.fi,
    placeholder: "مثال: فحص درجات حرارة التخزين"
  })), tbEl("div", {
    style: {
      display: "grid",
    ...
    value: p.icon,
    onChange: ie => T("icon", ie.target.value.slice(0, 4)),
    style: {
      ...a.fi,
      textAlign: "center",
      fontSize: 18
    ...
    style: a.fl
  }, "المنصب المسؤول"), tbEl("select", {
    value: p.role,
    onChange: ie => T("role", ie.target.value),
    style: a.fi
  }, TB_ROLES.map(ie => tbEl("option", {
    key: ie,
    value: ie
  }, qn[ie]))))), tbEl("div", {
    style: {
    ...
    value: p.category,
    onChange: ie => T("category", ie.target.value.slice(0, 60)),
    style: a.fi,
    placeholder: "التشغيل / الجودة / المبيعات",
    list: "tb-cats"
  }), tbEl("datalist", {
    id: "tb-cats"
  }, (W.categories || []).map(ie => tbEl("option", {
    key: ie,
    value: ie
  })))), tbEl("div", {
    style: a.fg
    ...
    style: a.fl
  }, "فترة التنفيذ"), tbEl("select", {
    value: p.period,
    onChange: ie => T("period", ie.target.value),
    style: a.fi
  }, Object.keys(TB_PERIODS).map(ie => tbEl("option", {
    key: ie,
    value: ie
  }, `${TB_PERIODS[ie].i} ${TB_PERIODS[ie].t}`))))), tbEl("div", {
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
      gap: 8
    ...
    style: a.fl
  }, "ترتيب الظهور"), tbEl("input", {
    type: "number",
    value: p.sortOrder,
    onChange: ie => T("sortOrder", ie.target.value),
    style: a.fi
  })), tbEl("div", {
    style: a.fg
  }, tbEl("label", {
    ...
    value: p.active ? "1" : "0",
    onChange: ie => T("active", ie.target.value === "1"),
    style: a.fi
  }, tbEl("option", {
    value: "1"
  }, "مفعّلة — تظهر للمنصب"), tbEl("option", {
    value: "0"
  }, "موقوفة — مخفية")))), tbEl("div", {
    style: a.fg
  }, tbEl("label", {
    style: a.fl
  }, "ملاحظة توضيحية"), tbEl("textarea", {
    value: p.notes,
    onChange: ie => T("notes", ie.target.value),
    style: {
      ...a.fi,
      minHeight: 56
    },
    placeholder: "تفاصيل تساعد المنفّذ — اختياري"
  })), W.error ? tbEl("div", {
    style: {
      background: "#ef444415",
      color: "#ef4444",
  ...
  }, W.error) : null, tbEl("button", {
    disabled: L || W.busy,
    onClick: () => W.onSubmit({
      ...p,
      sortOrder: tbNum(p.sortOrder) || 0
    }),
    style: {
      ...a.submitBtn,
      opacity: L || W.busy ? .55 : 1
    }
  }, W.busy ? "جارٍ الحفظ…" : "حفظ المهمة"))
}

/** نموذج مهمة مستعجلة — يحترم التسلسل الوظيفي */
function TbUrgentForm(W) {
  const Q = W.initial || {},
    p = tbBelowRoles(W.user.role),
    H = W.user.role === "ops_manager",
    T = W.user.role === "area_manager",
    [L, ie] = me.useState({
      title: Q.title || "",
      details: Q.details || "",
      priority: Q.priority || "متوسط",
      targetRole: Q.targetRole || p[0] || "",
      region: Q.region || (H ? "" : W.user.region || ""),
      branch: Q.branch || (W.user.role === "branch_manager" ? W.user.branch || "" : ""),
      dueDate: Q.dueDate || U()
    }),
    $ = (D, B) => ie(De => ({
      ...De,
    ...
    z = H ? L.region : T ? W.user.region || "" : W.user.region || "",
    oe = ((W.regions[z] || {}).branches || []),
    Ae = !L.title.trim() || !L.targetRole;
  return tbEl(me.Fragment, null, tbEl("div", {
    style: {
      background: "#f4f7f7",
      borderRadius: 10,
  ...
  }, `🏷️ أنت: ${qn[W.user.role]} — يمكنك الإسناد إلى: ${p.map(D=>qn[D]).join(" / ")||"لا يوجد منصب أدنى"}`), tbEl("div", {
    style: a.fg
  }, tbEl("label", {
    style: a.fl
  }, "عنوان المهمة *"), tbEl("input", {
    ...
    style: a.fi,
    placeholder: "مثال: إيقاف صنف نفد من المخزون"
  })), tbEl("div", {
    style: a.fg
  }, tbEl("label", {
    style: a.fl
  }, "التفاصيل"), tbEl("textarea", {
    value: L.details,
    onChange: D => $("details", D.target.value),
    style: {
      ...a.fi,
      minHeight: 60
    }
    ...
    style: a.fl
  }, "المنصب المُكلَّف *"), tbEl("select", {
    value: L.targetRole,
    onChange: D => $("targetRole", D.target.value),
    style: a.fi
  }, p.map(D => tbEl("option", {
    key: D,
    value: D
    ...
    style: a.fl
  }, "الأولوية"), tbEl("select", {
    value: L.priority,
    onChange: D => $("priority", D.target.value),
    style: a.fi
  }, Object.keys(TB_PRIO).map(D => tbEl("option", {
    key: D,
    value: D
  }, D))))), tbEl("div", {
    style: {
    ...
    value: H ? L.region : W.user.region || "",
    disabled: !H,
    onChange: D => {
      $("region", D.target.value), $("branch", "")
    },
    style: {
      ...a.fi,
      opacity: H ? 1 : .7
    ...
    value: ""
  }, "كل المناطق") : null, Object.entries(W.regions).map(([D, B]) => tbEl("option", {
    key: D,
    value: D
  }, B.name)))), tbEl("div", {
    style: a.fg
    ...
    value: L.branch,
    disabled: W.user.role === "branch_manager",
    onChange: D => $("branch", D.target.value),
    style: {
      ...a.fi,
      opacity: W.user.role === "branch_manager" ? .7 : 1
    }
  }, tbEl("option", {
    value: ""
  }, z ? "كل فروع المنطقة" : "كل الفروع"), oe.map(D => tbEl("option", {
    key: D.id,
    value: D.id
  }, D.name))))), tbEl("div", {
    style: a.fg
    ...
    style: a.fl
  }, "تاريخ الاستحقاق"), tbEl("input", {
    type: "date",
    value: L.dueDate,
    onChange: D => $("dueDate", D.target.value),
    style: a.fi
  })), W.error ? tbEl("div", {
    style: {
      background: "#ef444415",
    ...
    onClick: () => W.onSubmit({
      ...L,
      region: H ? L.region : W.user.region || ""
    }),
    style: {
      ...a.submitBtn,
      opacity: Ae || W.busy ? .55 : 1
    }
  }, W.busy ? "جارٍ الحفظ…" : "إرسال المهمة"))
}

/** نموذج الفرع — الموقع الجغرافي إلزامي لربطه بجداول الدوام */
function TbBranchForm(W) {
  const Q = W.initial || {},
    [p, H] = me.useState({
      region: Q.region || Object.keys(W.regions)[0] || "",
      name: Q.name || "",
      supervisor: Q.supervisor || "",
      lat: Q.lat != null ? String(Q.lat) : "",
      lng: Q.lng != null ? String(Q.lng) : "",
      radius: Q.radius != null ? String(Q.radius) : String(TB_DEF_RADIUS)
    }),
    [T, L] = me.useState(""),
    [ie, $] = me.useState(!1),
    z = (X, be) => H(wt => ({
    ...
    oe = tbNum(p.lat),
    Ae = tbNum(p.lng),
    D = oe != null && Ae != null && oe !== 0 && Ae !== 0,
    B = !p.name.trim() || !p.region || !D,
    De = () => {
      if (!navigator.geolocation) {
        L("المتصفح لا يدعم تحديد الموقع — أدخل الإحداثيات يدوياً");
        return
      }
      $(!0), L(""), navigator.geolocation.getCurrentPosition(X => {
        z("lat", X.coords.latitude.toFixed(6)), z("lng", X.coords.longitude.toFixed(6)), $(!1)
      }, () => {
        $(!1), L("تعذّر قراءة موقعك — تأكد من السماح بالوصول للموقع")
      }, {
  ...
  return tbEl(me.Fragment, null, tbEl("div", {
  ...
  }, "📍 الموقع الجغرافي إلزامي — عليه يعتمد ربط الفرع بجداول الدوام والتحقق من الحضور داخل النطاق."), tbEl("div", {
    style: {
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
      gap: 8
    ...
    value: p.region,
    onChange: X => z("region", X.target.value),
    style: a.fi
  }, Object.entries(W.regions).map(([X, be]) => tbEl("option", {
    key: X,
    value: X
  }, be.name)))), tbEl("div", {
    style: a.fg
    ...
    value: p.supervisor,
    onChange: X => z("supervisor", X.target.value),
    style: a.fi
  })), tbEl("button", {
    onClick: De,
    disabled: ie,
  ...
  }, ie ? "جارٍ تحديد الموقع…" : "📍 استخدام موقعي الحالي"), tbEl("div", {
    style: {
      display: "grid",
    ...
    value: p.lat,
    onChange: X => z("lat", X.target.value.trim()),
    style: a.fi,
    ...
    value: p.lng,
    onChange: X => z("lng", X.target.value.trim()),
    style: a.fi,
    placeholder: "39.192300",
    inputMode: "decimal"
  })), tbEl("div", {
    style: a.fg
  }, tbEl("label", {
    style: a.fl
  }, "النطاق (متر)"), tbEl("input", {
    value: p.radius,
    onChange: X => z("radius", X.target.value.trim()),
    style: a.fi,
    inputMode: "numeric"
  }))), D ? tbEl("a", {
    href: `https://www.google.com/maps?q=${oe},${Ae}`,
    target: "_blank",
    rel: "noreferrer",
    style: {
      display: "block",
  ...
  }, "🗺️ معاينة الموقع على الخريطة") : tbEl("p", {
    style: {
      fontSize: 11,
      color: "#ef4444",
      fontWeight: 600,
  ...
  }, "لن يُحفظ الفرع قبل تحديد الإحداثيات"), T || W.error ? tbEl("div", {
    style: {
      background: "#ef444415",
      color: "#ef4444",
      padding: "8px 12px",
  ...
  }, T || W.error) : null, tbEl("button", {
    disabled: B || W.busy,
    onClick: () => W.onSubmit({
      region: p.region,
      name: p.name.trim(),
      supervisor: p.supervisor.trim(),
      lat: oe,
      lng: Ae,
      radius: tbNum(p.radius) || TB_DEF_RADIUS
    }),
    style: {
      ...a.submitBtn,
      opacity: B || W.busy ? .55 : 1
    }
  }, W.busy ? "جارٍ الحفظ…" : W.initial ? "حفظ التعديلات" : "إضافة الفرع"))
}

/** ═══ خانة «مهامي» — العرض الجديد ═══ */
function TbTaskCenter(W) {
  const [Q, p] = me.useState("open"),
    [H, T] = me.useState("all"),
    L = W.viewRole,
    ie = L === W.user.role,
    $ = W.tasks.filter(X => X.active || W.isAdmin),
    z = X => W.doneMap[`${W.branchKey}-${X.id}`] === U(),
    oe = $.filter(X => X.active),
    Ae = oe.filter(z).length,
    D = oe.length ? Math.round(Ae / oe.length * 100) : 0,
    B = $.filter(X => {
      if (H !== "all" && X.period !== H) return !1;
      if (Q === "open") return !z(X) || !X.active;
      if (Q === "done") return z(X) && X.active;
      return !0
    }),
    De = tbGroupTasks(B),
    We = tbLastDays(7),
    ke = tbLastDays(30),
    we = X => W.hist[`${W.histKey}${X}`] || 0,
    bt = We.length ? Math.round(We.reduce((X, be) => X + we(be), 0) / We.length) : 0,
    nt = ke.length ? Math.round(ke.reduce((X, be) => X + we(be), 0) / ke.length) : 0,
    jt = (W.urgent || []).filter(X => X.status !== "مكتمل"),
    qe = [...new Set(W.allTasks.map(X => X.category))].sort((X, be) => X.localeCompare(be, "ar"));
  ...
  const Nt = tbEl("div", {
    ...
    et = tbEl("div", {
      ...
      return tbEl("div", {
        ...
        style: {
          width: "100%",
          height: `${Math.max(4,be*.62)}px`,
          borderRadius: 6,
          background: be ? xe(be) : "#eef2f2",
        ...
        style: {
          fontSize: 9,
          color: X === U() ? v.t : "#bbb",
          fontWeight: X === U() ? 800 : 600
        }
      }, tbDayName[wt]))
    }))),
    ct = jt.length ? tbEl("div", {
      style: {
        background: "#fff",
    ...
    }, `⚡ مهام مستعجلة موجَّهة لك (${jt.length})`), tbEl("button", {
      onClick: W.openUrgentTab,
      style: {
        ...a.stBtn,
        color: v.t,
        fontSize: 11
      ...
      style: {
        ...
        height: 8,
        borderRadius: "50%",
        background: TB_PRIO[X.priority] || v.t,
        flexShrink: 0
      }
    }), tbEl("span", {
      style: {
    ...
    }, X.title), X.dueDate ? tbEl("span", {
      style: {
        fontSize: 10,
        color: X.dueDate < U() ? "#ef4444" : "#aaa",
        fontWeight: 600
      }
    }, `📅 ${X.dueDate}`) : null, tbEl("button", {
      onClick: () => W.onUrgentStatus(X, "مكتمل"),
      style: {
        ...a.stBtn,
        color: "#22c55e",
        fontSize: 11,
    ...
    }, "⚙️ إدارة كتالوج المهام"), tbEl("button", {
      onClick: () => W.onAdd(L),
      style: {
        ...a.addBtn,
        fontSize: 11.5
    ...
    }, TB_ROLES.map(X => tbEl("button", {
      key: X,
      onClick: () => W.setViewRole(X),
      style: {
        padding: "5px 12px",
        borderRadius: 20,
        border: `1px solid ${L===X?v.t:"#e8eded"}`,
        background: L === X ? v.t : "#fff",
        color: L === X ? "#fff" : "#777",
        fontSize: 11.5,
        fontWeight: 600,
        cursor: "pointer"
      }
    }, qn[X], X === W.user.role ? " (مهامي)" : ""))), W.online ? null : tbEl("p", {
      style: {
        fontSize: 11,
        color: "#f59e0b",
        fontWeight: 600,
    ...
    }, "⚠️ تعذّر الاتصال بقاعدة البيانات — تُعرض المهام الافتراضية والتعديل معطّل"), W.msg ? tbEl("p", {
      style: {
        fontSize: 11,
        color: "#ef4444",
        fontWeight: 600,
    ...
    }, Q === "done" ? "لم يُنجز شيء بعد اليوم" : "لا توجد مهام مطابقة") : De.map(([X, be]) => {
      const wt = be.filter(z).length;
      return tbEl("div", {
        key: X,
        style: a.card
      }, tbEl("div", {
        ...
        style: {
          width: `${be.length?wt/be.length*100:0}%`,
          height: "100%",
          background: v.t,
          transition: "width .4s"
        }
        ...
        return tbEl("div", {
          ...
          style: {
            ...
            padding: "8px 10px",
            borderRadius: 8,
            border: `1px solid ${c?"#22c55e30":"#eef2f2"}`,
            background: c ? "#f4fbf6" : "#fff",
            opacity: Zt.active ? 1 : .55
          }
        }, tbEl("button", {
          onClick: () => ie && Zt.active && W.toggle(Zt.id),
          disabled: !ie || !Zt.active,
          title: ie ? "" : "أنت تعرض مهام منصب آخر",
          style: {
            width: 22,
            height: 22,
            borderRadius: 6,
            border: `2px solid ${c?"#22c55e":"#D0E0E0"}`,
            background: c ? "#22c55e" : "#fff",
            color: "#fff",
            fontSize: 12,
            fontWeight: 800,
            cursor: ie && Zt.active ? "pointer" : "default",
            flexShrink: 0,
            lineHeight: 1
          }
        }, c ? "✓" : ""), tbEl("span", {
        ...
        }, Zt.title), Zt.notes ? tbEl("p", {
          style: {
            fontSize: 10.5,
            color: "#aab",
            margin: "2px 0 0"
          }
        }, Zt.notes) : null), Zt.active ? null : tbEl("span", {
          style: {
            ...a.bdg,
            background: "#ef444415",
            color: "#ef4444"
        ...
        }, TB_PERIODS[Zt.period].i), W.isAdmin && W.online ? tbEl(me.Fragment, null, tbEl("button", {
          onClick: () => W.onEdit(Zt),
          style: {
            background: "none",
            border: "none",
  ...
  return tbEl(me.Fragment, null, tbEl(TbHero, {
    title: "✅ مهامي",
    subtitle: `${qn[W.user.role]} — ${En(U())}`,
    note: ie ? `${Ae} من ${oe.length} مهمة منجزة اليوم` : `تعرض حالياً مهام: ${qn[L]}`,
    pct: D,
    done: Ae,
    total: oe.length
  }), ct, Nt, et, It, tbEl(TbChips, {
    value: Q,
    onPick: p,
    items: [{
      k: "open",
    ...
    items: [{
      k: "all",
      t: "كل الفترات"
    }, ...Object.keys(TB_PERIODS).map(X => ({
      k: X,
      t: `${TB_PERIODS[X].i} ${TB_PERIODS[X].t}`
    }))]
  }), on, W.modal === "tb_task_form" && W.draft ? tbEl(mn, {
    close: W.closeModal,
    title: W.draft.mode === "edit" ? "تعديل مهمة" : "مهمة جديدة"
  }, tbEl(TbTaskForm, {
    initial: W.draft.mode === "edit" ? W.draft.task : null,
    defaultRole: L,
    categories: qe,
    nextOrder: W.allTasks.length,
    busy: W.busy,
    error: W.msg,
    onSubmit: W.onSubmitTask
  })) : null, W.modal === "tb_task_delete" && W.draft ? tbEl(mn, {
    close: W.closeModal,
    title: "حذف مهمة"
  }, tbEl("div", null, tbEl("p", {
    style: {
      fontSize: 13,
      color: v.d,
      margin: "0 0 6px"
    }
  }, `سيتم حذف المهمة «${W.draft.task.title}» نهائياً من كتالوج ${qn[W.draft.task.role]}.`), W.msg ? tbEl("p", {
    style: {
      fontSize: 11.5,
      color: "#ef4444",
      fontWeight: 600
    ...
    disabled: W.busy,
    onClick: W.onConfirmDelete,
    style: {
      ...a.submitBtn,
      background: "#ef4444",
      opacity: W.busy ? .6 : 1
    }
  }, W.busy ? "جارٍ الحذف…" : "تأكيد الحذف"), tbEl("button", {
...
/** ═══ خانة «المهام المستعجلة» — حسب التسلسل الوظيفي ═══ */
function TbUrgentCenter(W) {
  const H = W.tasks || [],
    T = H.filter(D => D.assignedToMe),
    L = H.filter(D => !D.assignedToMe && D.canManage),
    [Q, p] = me.useState(T.length || !L.length ? "mine" : "sent"),
    ie = tbBelowRoles(W.user.role),
    $ = Q === "mine" ? T : Q === "sent" ? L : H,
    z = $.filter(D => D.status !== "مكتمل").sort((D, B) => (TB_LEVEL[D.targetRole] - TB_LEVEL[B.targetRole]) || (D.dueDate || "9999").localeCompare(B.dueDate || "9999")),
    oe = $.filter(D => D.status === "مكتمل"),
    Ae = D => {
      const B = D.region ? (W.regions[D.region] || {}).name || D.region : "كل المناطق",
        De = D.branch ? (((W.regions[D.region] || {}).branches || []).find(We => We.id === D.branch) || {}).name || D.branch : "";
      return De ? `${B} • ${De}` : B
    };
  return tbEl(me.Fragment, null, tbEl(TbHero, {
    title: "⚡ المهام المستعجلة",
    subtitle: `${qn[W.user.role]} — التوزيع حسب التسلسل الوظيفي`,
    note: `${T.filter(D=>D.status!=="مكتمل").length} مهمة مسندة لك • ${L.filter(D=>D.status!=="مكتمل").length} مهمة لدى مرؤوسيك`,
    pct: $.length ? Math.round(oe.length / $.length * 100) : 0,
    done: oe.length,
    total: $.length
  }), tbEl("div", {
    style: {
  ...
  }), ie.length ? tbEl("button", {
    onClick: W.onAdd,
    style: a.addBtn
  }, "+ مهمة مستعجلة") : null), W.online ? null : tbEl("p", {
    style: {
      fontSize: 11.5,
      color: "#f59e0b",
      fontWeight: 600,
  ...
  }, "⚠️ تعذّر الاتصال بقاعدة البيانات — تحديث المهام المستعجلة غير متاح حالياً"), W.msg ? tbEl("p", {
    style: {
      fontSize: 11.5,
      color: "#ef4444",
      fontWeight: 600,
  ...
  }, W.msg) : null, ie.length ? null : tbEl("p", {
    style: {
      fontSize: 11.5,
      color: "#999",
      marginBottom: 8
    }
  }, "منصبك ينفّذ المهام المستعجلة ولا يُسندها."), z.length + oe.length === 0 ? tbEl("p", {
    style: {
      textAlign: "center",
      color: "#aaa",
      padding: 30,
  ...
  }, "لا توجد مهام مستعجلة") : tbEl("div", {
    style: {
      display: "flex",
      flexDirection: "column",
      gap: 8
    }
  }, z.map(D => tbEl(TbUrgentCard, {
    key: D.id,
    task: D,
    busy: W.busy,
    scopeText: Ae(D),
  ...
  }, `✓ المنجزة (${oe.length})`) : null, oe.map(D => tbEl(TbUrgentCard, {
    key: D.id,
    task: D,
    busy: W.busy,
    scopeText: Ae(D),
    ...
    onEdit: W.onEdit,
    onDelete: W.onDelete
  }))), W.modal === "tb_urgent_form" && W.draft ? tbEl(mn, {
    close: W.closeModal,
    title: W.draft.mode === "edit" ? "تعديل مهمة مستعجلة" : "مهمة مستعجلة جديدة"
  }, tbEl(TbUrgentForm, {
    user: W.user,
    regions: W.regions,
    initial: W.draft.mode === "edit" ? W.draft.task : null,
    busy: W.busy,
    error: W.msg,
    onSubmit: W.onSubmitUrgent
  })) : null, W.modal === "tb_urgent_delete" && W.draft ? tbEl(mn, {
    close: W.closeModal,
    title: "حذف مهمة مستعجلة"
  }, tbEl("div", null, tbEl("p", {
    style: {
      fontSize: 13,
      color: v.d,
  ...
  }, `سيتم حذف «${W.draft.task.title}» نهائياً.`), tbEl("div", {
    style: {
      display: "flex",
      gap: 8
    }
...
function cf() {
  ...
  const tkLoad = async () => {
      try {
        const Y = await tbApi("/api/daily-tasks", "GET", T);
        return tkSetRows((Y.tasks || []).map(q => tbNormTask(q))), tkSetOnline(!0), !0
      } catch {
        return tkSetOnline(!1), !1
      }
    },
    tkMutate = async (Y, q, Se) => {
      tkSetBusy(!0), tkSetMsg("");
      try {
        await tbApi(Y, q, T, Se), await tkLoad(), tkSetDraft(null), pe(null)
      } catch (Be) {
        tkSetMsg(Be && Be.message || "تعذّر تنفيذ الطلب")
      } finally {
        tkSetBusy(!1)
      }
    },
    ugLoad = async () => {
      try {
        const Y = await tbApi("/api/urgent-tasks", "GET", T);
        return ugSetRows(Y.tasks || []), ugSetOnline(!0), !0
      } catch {
        return ugSetOnline(!1), !1
      }
    },
    ugMutate = async (Y, q, Se) => {
      ugSetBusy(!0), ugSetMsg("");
      try {
        await tbApi(Y, q, T, Se), await ugLoad(), ugSetDraft(null), pe(null)
      } catch (Be) {
        ugSetMsg(Be && Be.message || "تعذّر تنفيذ الطلب")
      } finally {
        ugSetBusy(!1)
      }
    },
    brLoad = async () => {
      try {
        const Y = await tbApi("/api/branches", "GET", T);
        return H(q => tbMergeBranches(q, Y.branches || [])), brSetOnline(!0), !0
      } catch {
        return brSetOnline(!1), !1
      }
    },
    brMutate = async (Y, q, Se) => {
      brSetBusy(!0), brSetMsg("");
      try {
        await tbApi(Y, q, T, Se), await brLoad(), brSetDraft(null), pe(null)
      } catch (Be) {
        brSetMsg(Be && Be.message || "تعذّر تنفيذ الطلب")
      } finally {
        brSetBusy(!1)
      }
    };
  ...
  const O = T,
    ...
    tkAll = tkRows || tbFallbackTasks(),
    tkFor = Y => tbSortTasks(tkAll.filter(q => q.role === Y)),
    tkViewRole = _e && tkRoleView ? tkRoleView : O.role,
    tkByRole = TB_ROLES.reduce((Y, q) => (Y[q] = tkFor(q), Y), {}),
    ugMine = ugRows.filter(Y => Y.assignedToMe),
    ft = tkFor(O.role).filter(Y => Y.active),
    yn = O.branch || "hq",
    Mt = ft.filter(Y => B[`${yn}-${Y.id}`] === U()).length,
    $e = ft.length ? Math.round(Mt / ft.length * 100) : 0,
    Tn = ugMine.filter(Y => Y.status !== "مكتمل").length,
...
`, Object.entries(tkByRole).forEach(([X, be]) => {
        be.forEach(wt => {
          ot.forEach(Zt => {
...
`, ugRows.forEach(X => {
        q += `"${X.title}","${X.priority}","${X.status}","${X.dueDate||(X.createdAt||"").slice(0,10)}","${X.createdByName||""}","${qn[X.targetRole]||""}"
`
...
`);
    ...
    Nl = () => {
          ...
          children: [l.jsxs("h1", {
            ...
            children: [l.jsxs("div", {
              ...
              children: [l.jsx("h3", {
                ...
                l: "مهام مستعجلة",
                done: ugRows.filter(y => y.status === "مكتمل").length,
                total: ugRows.length,
                c: "#ef4444"
              }, {
                l: "طلبات صيانة",
      ...
      if (z === "tasks") return tbEl(TbTaskCenter, {
        user: O,
        isAdmin: _e,
        viewRole: tkViewRole,
        setViewRole: tkSetRoleView,
        tasks: tkFor(tkViewRole),
        allTasks: tkAll,
        doneMap: B,
        branchKey: yn,
        toggle: Gr,
        ...
        msg: tkMsg,
        urgent: ugMine,
        onUrgentStatus: (c, y) => ugMutate(`/api/urgent-tasks/${c.id}`, "PUT", {
          status: y
        }),
        openUrgentTab: () => oe("urgent"),
        onAdd: c => {
          tkSetMsg(""), tkSetDraft({
            mode: "add",
            role: c
        ...
        draft: tkDraft,
        closeModal: () => {
          pe(null), tkSetDraft(null), tkSetMsg("")
        },
        onSubmitTask: c => tkDraft && tkDraft.mode === "edit" && tkDraft.task.dbId != null ? tkMutate(`/api/daily-tasks/${tkDraft.task.dbId}`, "PUT", c) : tkMutate("/api/daily-tasks", "POST", c),
        onConfirmDelete: () => tkDraft && tkDraft.task.dbId != null ? tkMutate(`/api/daily-tasks/${tkDraft.task.dbId}`, "DELETE") : tkSetMsg("هذه مهمة افتراضية غير محفوظة في قاعدة البيانات — تعذّر حذفها")
      });
      if (z === "reports_req") {
      ...
      if (z === "urgent") return tbEl(TbUrgentCenter, {
        user: O,
        regions: p,
        tasks: ugRows,
        online: ugOnline,
        busy: ugBusy,
        msg: ugMsg,
        onStatus: (c, y) => ugMutate(`/api/urgent-tasks/${c.id}`, "PUT", {
          status: y
        }),
        onAdd: () => {
          ugSetMsg(""), ugSetDraft({
        ...
        draft: ugDraft,
        closeModal: () => {
          pe(null), ugSetDraft(null), ugSetMsg("")
        },
        onSubmitUrgent: c => ugDraft && ugDraft.mode === "edit" ? ugMutate(`/api/urgent-tasks/${ugDraft.task.id}`, "PUT", c) : ugMutate("/api/urgent-tasks", "POST", c),
        onConfirmDelete: () => ugDraft && ugMutate(`/api/urgent-tasks/${ugDraft.task.id}`, "DELETE")
      });
      if (z === "tracking") {
              ...
              children: [l.jsxs("div", {
                ...
                children: [l.jsx("span", {
                  ...
                  children: [l.jsx("span", {
                    ...
                    children: ["الفرع المحدد: ", Z.branch || "—", amBranchId && tbGeoOf(p, amBranchId) ? ` • نطاق ${tbGeoOf(p,amBranchId).radius} متر` : " • لا توجد إحداثيات للفرع — حدّدها من إدارة النظام"]
                  })]
                })]
              }), l.jsxs("div", {
                style: {
      ...
      if (z === "admin") return l.jsxs(l.Fragment, {
        children: [l.jsx("h1", {
        ...
        }, "+ فرع جديد"), tbBranchesNoGeo(p).length ? tbEl("div", {
          style: {
            background: "#f59e0b12",
        ...
        }, `⚠️ ${tbBranchesNoGeo(p).length} فرع بلا موقع جغرافي — لن يعمل التحقق من الحضور داخل النطاق في جداول الدوام حتى تُحدَّد إحداثياته.`) : null, Object.entries(p).map(([c, y]) => tbEl("div", {
          key: c,
          style: {
            ...a.card,
            marginBottom: 10
        ...
        }, `${(y.branches||[]).length} فرع`)), (y.branches || []).map(j => {
          const S = tbGeoOf(p, j.id);
          return tbEl("div", {
            key: j.id,
            style: {
              display: "flex",
            ...
            style: {
              fontSize: 10,
              color: S ? "#8a9797" : "#ef4444",
              margin: "2px 0 0",
              fontWeight: S ? 500 : 700
            }
          }, S ? `📍 ${S.lat.toFixed(4)}, ${S.lng.toFixed(4)} • نطاق ${S.radius} م` : "📍 بلا موقع جغرافي — غير مربوط بجداول الدوام")), tbEl("button", {
            onClick: () => {
              brSetMsg(""), brSetDraft({
                mode: "edit",
                branch: {
            ...
            onClick: () => j.dbId != null && brOnline ? brMutate(`/api/branches/${j.dbId}`, "DELETE") : H(_ => ({
              ..._,
              [c]: {
                ..._[c],
                branches: _[c].branches.filter(Z => Z.id !== j.id)
              }
            })),
            style: {
              background: "none",
          ...
          close: () => {
            pe(null), brSetDraft(null), brSetMsg("")
          },
          title: brDraft && brDraft.mode === "edit" ? `تعديل الفرع — ${brDraft.branch.name}` : "فرع جديد",
          children: tbEl(TbBranchForm, {
            regions: p,
            initial: brDraft && brDraft.mode === "edit" ? brDraft.branch : null,
            busy: brBusy,
            error: brMsg,
            onSubmit: c => {
              const y = brDraft && brDraft.mode === "edit" ? brDraft.branch : null;
              if (brOnline && (!y || y.dbId != null)) return y ? brMutate(`/api/branches/${y.dbId}`, "PUT", c) : brMutate("/api/branches", "POST", c);
              H(j => {
                const S = {};
                Object.keys(j).forEach(_ => S[_] = {
                  ...j[_],
                  branches: (j[_].branches || []).filter(Z => !y || Z.id !== y.id)
                });
                const Z = y ? {
                  ...y,
                  ...c,
                ...
                return S[c.region] = {
                  ...S[c.region],
                  branches: [...(S[c.region].branches || []), Z]
                }, S
              }), pe(null), brSetDraft(null), brSetMsg("")
            }
          })
